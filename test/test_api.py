...
def test_edit_cart():
    ...
    """
    edit_cart_api = UpdateCartAPI(
        API2_url)  # Создаем экземпляр класса API для редактирования корзины

    product_id = 2967760  # ID продукта для добавления
    item_list_name = "search"  # Имя списка, откуда добавляется продукт
    add_to_cart_api = AddToCartAPI(API1_url)
    status_code = add_to_cart_api.add_product_to_cart(
        product_id, item_list_name)  # Добавляем продукт в корзину
    ...
    items_to_update = [{'id': 2841859, "quantity": 2}]

    # Редактируем корзину
    update_cart_response = edit_cart_api.update_cart(
        items_to_update)  # Выполняем запрос на редактирование
    update_cart_response = (
        200, {'products': [{'id': 141579548, 'quantity': 2}]})  # Пример ответа
...
def test_wrong_add_request():
    ...
    with allure.step("Попытка добавить книгу в корзину некорректно"):
        ...
        status_code = wrong_add_api.wrong_add_product(
            product_id, item_list_name)  # Выполняем неверный запрос
...
def test_add_product_to_cart_with_empty_body():
    ...
    with allure.step("Проверить статус запроса"):
        ...
        assert status_code == 422, "Ожидается статус 422 Unprocessable Entity, но получен статус {}".format(
            status_code)
